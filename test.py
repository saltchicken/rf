...
import asyncio
import numpy as np
from rf import ReaderRecorder, get_args
...
def is_speech_from_features(features):
    ...
    print(
        f"ZCR: {zcr:.3f}, Energy: {energy:.3f}, Centroid: {spec_centroid:.1f}, Entropy: {spec_entropy:.3f}"
    )
    ...
    if (
        0.02 < zcr < 0.2
        and energy > 0.01
        and 300 < spec_centroid < 2000
        and spec_entropy < 0.85
    ):
...
def main():
    ...
    config.read(f"./src/rf/config/config.ini")
    ...
    for index, result in enumerate(results):
        ...
        print(
            f"Freq: {centered_freq_offsets[index] + int(center_freq)}, Label: {top_label}, Score: {top_score}"
        )
